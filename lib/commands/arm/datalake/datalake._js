...
exports.init = function (cli) {
  ...
  var dataLakeFileSystem = dataLakeCommands.category('filesystem')
  ...
  dataLakeFileSystem.command('list [dataLakeAccountName] [path]')
    ...
    .execute(function (dataLakeAccountName, path, value, folder, force, options, _) {
	  ...
	  var client = utils.createDataLakeFileSystemManagementClient(subscription, options);
  ...
  dataLakeFileSystem.command('import [dataLakeAccountName] [path] [destination] [force]')
    .description($('Uploads the specified the specified file, to the target destination in an Azure Data Lake.'))
	.usage('[options] <dataLakeAccountName> <path> <destination>')
	.option('-n --dataLakeAccountName <dataLakeAccountName>', $('the Data Lake account name to execute the action on'))
      ...
      function (log, _) {
		  ...
		  try {
			  ...
			  while(offset < fileSizeInBytes ) {
				  ...
				  if(offset === 0) {
					  var response = client.fileSystem.internalBeginCreate(destination, dataLakeAccountName, parameters, _);
					  client.fileSystem.create(response.location, buffer, _);
				  }
				  else {
					 var response = client.fileSystem.internalBeginAppend(destination, dataLakeAccountName, null, _);
  ...
  dataLakeFileSystem.command('export [dataLakeAccountName] [path] [destination] [force]')
  ...
  var dataLakeFileSystemPermissions = dataLakeCommands.category('permissions')
 ...
 dataLakeFileSystemPermissions.command('delete [dataLakeAccountName] [path] [defaultAcl]')
  ...
  var dataLakeAccount = dataLakeCommands.category('account')
