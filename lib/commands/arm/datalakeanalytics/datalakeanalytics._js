...
exports.init = function (cli) {
  ...
  var dataLakeCommands = cli.category('datalake')
    .description($('Commands to interact with Azure Data Lake'));
  
  var dataLakeAnalyticsCommands = dataLakeCommands.category('analytics')
    .description($('Commands to interact with Azure Data Lake Analytics'));
  ...
  dataLakeAnalyticsJob.command('create [dataLakeAnalyticsAccountName] [jobName] [script] [runtime] [compileMode] [compileOnly] [degreeOfParallelism] [priority] [resourceGroup]' )
    ...
    .option('-n --dataLakeAnalyticsAccountName <dataLakeAnalyticsAccountName>', $('the Data Lake Analytics account name to execute the action on'))
    ...
    .option('-g --resourceGroup <resourceGroup>', $('the optional resource group to force lookup of the Data Lake Analytics account in.'))
  ...
  dataLakeAnalyticsJob.command('list [dataLakeAnalyticsAccountName] [jobName] [submitter] [submittedAfter] [submittedBefore] [state] [result] [resourceGroup]' )
    .description($('List jobs, with filter criteria for dates, job completion state, as well as submitter.'))
    .usage('[options] <dataLakeAnalyticsAccountName> <jobId> <includeDebugInfo> <includeStatistics> <resourceGroup>')
    .option('-n --dataLakeAnalyticsAccountName <dataLakeAnalyticsAccountName>', $('the Data Lake Analytics account name to execute the action on'))
  ...
  dataLakeAnalyticsCatalog.command('list [dataLakeAnalyticsAccountName] [catalogItemType] [catalogItemPath] [resourceGroup]')
    ...
    .option('-n --dataLakeAnalyticsAccountName <dataLakeAnalyticsAccountName>', $('The Data Lake Analytics account name to perform the action on.'))
  ...
  dataLakeAnalyticsCatalog.command('createsecret [dataLakeAnalyticsAccountName] [databaseName] [hostUri] [secretName] [resourceGroup]')
    .description($('Creates the specified secret for the specified database.  Secrets are used for credentials to remote datasources, like Azure SQL Databases.'))
    .usage('[options] <dataLakeAnalyticsAccountName> <databaseName> <hostUri> <secretName> <resourceGroup>')
    .option('-n --dataLakeAnalyticsAccountName <dataLakeAnalyticsAccountName>', $('The Data Lake Analytics account name to perform the action on.'))
    .option('-d --databaseName <databaseName>', $('The name of the database (in the Data Lake Analytics Catalog) to create the secret in.'))
    ...
    .usage('[options] <dataLakeAnalyticsAccountName> <databaseName> <hostUri> <secretName> <resourceGroup>')
    .option('-n --dataLakeAnalyticsAccountName <dataLakeAnalyticsAccountName>', $('The Data Lake Analytics account name to perform the action on.'))
    .option('-d --databaseName <databaseName>', $('The name of the database (in the Data Lake Analytics Catalog) to delete the secret from.'))
  ...
  dataLakeAnalyticsAccount.command('show [dataLakeAnalyticsAccountName] [resourceGroup]')
    ...
    .option('-n --dataLakeAnalyticsAccountName <dataLakeAnalyticsAccountName>', $('the Data Lake Analytics account name to retrieve'))
    .option('-g --resourceGroup <resourceGroup>', $('the optional resource group to list the accounts in'))
    ...
    .usage('[options] <dataLakeAnalyticsAccountName> <resourceGroup>')
    .option('-n --dataLakeAnalyticsAccountName <dataLakeAnalyticsAccountName>', $('the Data Lake Analytics account name to delete'))
    .option('-g --resourceGroup <resourceGroup>', $('the optional resource group to force the command to find the Data Lake Analytics account to delete in.'))
    .option('-q, --quiet', $('quiet mode (do not ask for delete confirmation)'))
    .execute(function (dataLakeAnalyticsAccountName, resourceGroup, options, _) {
      if (!dataLakeAnalyticsAccountName) {
        return cli.missingArgument('dataLakeAnalyticsAccountName');
      ...
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete Data Lake Analytics Account %s? [y/n] '), dataLakeAnalyticsAccountName), _)) {
        return;
      }
    ...
    .option('-n --dataLakeAnalyticsAccountName <dataLakeAnalyticsAccountName>', $('The Data Lake Analytics account name to create'))
    ...
    .option('-n --dataLakeAnalyticsAccountName <dataLakeAnalyticsAccountName>', $('The Data Lake Analytics account name to perform the action on.'))
    .option('-l --dataLakeStore <dataLakeStore>', $('the data lake storage account to add. For queries against this account to succeed, a user must have permissions on the Data Lake Store source. NOTE: this argument and --isDefaultDataLakeStore are part of a parameter set, and cannot be specified with --azureBlob and --accessKey.'))
    ...
    .option('-b --azureBlob <azureBlob>', $('the azure blob account name to add to the account. NOTE: this argument and --accessKey are part of a parameter set, and cannot be specified with --dataLakeStore and --isDefaultDataLakeStore.'))
